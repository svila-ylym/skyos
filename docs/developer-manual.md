...
src/gui/*.asm         NASM %include GUI modules (terminal, browser, taskmgr, notepad, file actions)
...
`src/kernel.asm` keeps boot, interrupt, device, filesystem, networking, shell,
and core GUI orchestration in one translation unit. GUI application code is now
split into NASM include modules under `src/gui/` and pulled together with
`%include` so symbols still resolve in a single kernel link.

Current split:

```text
src/gui/terminal.asm   framebuffer terminal rendering and shell-like history line
src/gui/browser.asm    address bar input and simple HTML/network page routing
src/gui/taskmgr.asm    Windows-style process/performance task manager view
src/gui/notepad.asm    Notepad-style text editor surface
src/gui/files.asm      file manager row selection and open-with routing
```

Older `src/kernel.asm` sections are organized into:
...
FAT32/exFAT/NTFS/ext signature probes for interoperability checks
...
The live ISO starts in the shell. Enter `desktop`, `gui`, or `startx` to launch
the framebuffer desktop. BIOS El Torito and GRUB Multiboot paths are generated;
`EFI/BOOT` is staged for a future `BOOTIA32.EFI` loader.
