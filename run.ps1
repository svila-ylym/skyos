...
$SkyOsVersion = "1.0.1.0.SKYCNQU"
$SkyOsInternalVersion = "10002"
$SkyOsSystemId = "skyos-10002"
...
function Initialize-IsoPayload {
    ...
    Write-TextFile (Join-Path $rootfs "usr\share\doc\skyos\README") "This root filesystem is staged for ISO inspection and future initramfs/rootfs mounting.`n"
    ...
    Copy-Item -LiteralPath (Join-Path $rootfs "*") -Destination $IsoRoot -Recurse -Force
    ...
    Write-TextFile (Join-Path $IsoRoot "rootfs\MANIFEST") "rootfs.tar.gz: compressed SkyOS root filesystem archive`nlayout: /bin /sbin /etc /usr /lib /var /home /dev /proc /sys`n"
    ...
    Write-TextFile (Join-Path $IsoRoot "install\INSTALL.md") "SkyOS installer stage. Boot the ISO, run installer to review display/timezone/disk state, then use settings display auto, timezone set, disk select 0, disk partition 4, disk format 0, and install. Device names are exposed as /dev/disk0 and /dev/disk0p0..p3 in the current IDE driver stage. The installer formats the target SkyFS partition before writing rootfs metadata and files, then writes MBR stage1, stage2, kernel ELF, and SkyFS v0 metadata so the disk can boot without the ISO.`n"
    ...
    $packagesIndex = @(
        "Package: base-tools`nVersion: $SkyOsVersion`nInternalVersion: $SkyOsInternalVersion`nArchitecture: i386`nSystem: $SkyOsSystemId`nCompatibleInternalVersion: $SkyOsInternalVersion`nFilename: pool/main/base-tools_1.0.1.0.SKYCNQU_skyos-i386.spk`nSize: 318`nFormat: spk-binary-v1`nDescription: Base SkyOS shell tools",
        "Package: skycrt`nVersion: $SkyOsVersion`nInternalVersion: $SkyOsInternalVersion`nArchitecture: i386`nSystem: $SkyOsSystemId`nCompatibleInternalVersion: $SkyOsInternalVersion`nFilename: pool/main/skycrt_1.0.1.0.SKYCNQU_skyos-i386.sxr`nSize: 256`nFormat: sxr-binary-v1`nRuntime: sxr`nPackageType: sxr-library`nDescription: SkyOS native runtime SXR",
        "Package: skyui`nVersion: $SkyOsVersion`nInternalVersion: $SkyOsInternalVersion`nArchitecture: i386`nSystem: $SkyOsSystemId`nCompatibleInternalVersion: $SkyOsInternalVersion`nSXR-Depends: skycrt (>= $SkyOsInternalVersion)`nFilename: pool/main/skyui_1.0.1.0.SKYCNQU_skyos-i386.sxr`nSize: 288`nFormat: sxr-binary-v1`nRuntime: sxr`nPackageType: sxr-library`nDescription: SkyOS GUI runtime SXR",
        "Package: calc`nVersion: $SkyOsVersion`nInternalVersion: $SkyOsInternalVersion`nArchitecture: i386`nSystem: $SkyOsSystemId`nCompatibleInternalVersion: $SkyOsInternalVersion`nSXR-Depends: skycrt (>= $SkyOsInternalVersion)`nFilename: pool/main/calc_1.0.1.0.SKYCNQU_skyos-i386.spk`nSize: 304`nFormat: spk-binary-v2`nRuntime: native-i386`nEntryPoint: /usr/bin/calc`nCommands: calc`nPackageType: cli-app`nPermissions: {read:[/home],write:[/home/user]}`nTuringComplete: yes`nDescription: Calc for SkyOS",
        "Package: skygui-demo`nVersion: $SkyOsVersion`nInternalVersion: $SkyOsInternalVersion`nArchitecture: i386`nSystem: $SkyOsSystemId`nCompatibleInternalVersion: $SkyOsInternalVersion`nDepends: base-tools (>= $SkyOsInternalVersion)`nSXR-Depends: skyui (>= $SkyOsInternalVersion), skycrt (>= $SkyOsInternalVersion)`nFilename: pool/main/skygui-demo_1.0.1.0.SKYCNQU_skyos-i386.spk`nSize: 512`nFormat: spk-binary-v2`nRuntime: soj`nEntryPoint: /opt/skygui-demo/install.soj`nCommands: skygui-demo`nPackageType: installer`nPermissions: {read:[/home,/usr/share],write:[/home/user,/tmp]}`nTuringComplete: yes`nDescription: GUI installer demo package",
        "Package: editor-vi`nVersion: $SkyOsVersion`nInternalVersion: $SkyOsInternalVersion`nArchitecture: i386`nSystem: $SkyOsSystemId`nCompatibleInternalVersion: $SkyOsInternalVersion`nDepends: base-tools (>= $SkyOsInternalVersion)`nFilename: pool/main/editor-vi_1.0.1.0.SKYCNQU_skyos-i386.spk`nSize: 339`nFormat: spk-binary-v1`nDescription: SkyOS vi editor package",
        "Package: net-tools`nVersion: $SkyOsVersion`nInternalVersion: $SkyOsInternalVersion`nArchitecture: i386`nSystem: $SkyOsSystemId`nCompatibleInternalVersion: $SkyOsInternalVersion`nDepends: base-tools (>= $SkyOsInternalVersion)`nFilename: pool/main/net-tools_1.0.1.0.SKYCNQU_skyos-i386.spk`nSize: 342`nFormat: spk-binary-v1`nDescription: Network commands for SkyOS",
        "Package: sapp-utils`nVersion: $SkyOsVersion`nInternalVersion: $SkyOsInternalVersion`nArchitecture: i386`nSystem: $SkyOsSystemId`nCompatibleInternalVersion: $SkyOsInternalVersion`nDepends: base-tools (>= $SkyOsInternalVersion), net-tools (>= $SkyOsInternalVersion)`nFilename: pool/main/sapp-utils_1.0.1.0.SKYCNQU_skyos-i386.spk`nSize: 369`nFormat: spk-binary-v1`nDescription: Sapp package manager utilities"
    ) -join "`n`n"
    Write-TextFile (Join-Path $IsoRoot "dists\skyos\main\binary-i386\Packages") ($packagesIndex + "`n")
    Write-TextFile (Join-Path $IsoRoot "pool\main\base-tools_1.0.1.0.SKYCNQU_skyos-i386.spk") "SkyOS Sapp package placeholder: base-tools`n"
    Write-TextFile (Join-Path $IsoRoot "pool\main\calc_1.0.1.0.SKYCNQU_skyos-i386.spk") "SkyOS Sapp package placeholder: calc`n"
    Write-TextFile (Join-Path $IsoRoot "pool\main\skycrt_1.0.1.0.SKYCNQU_skyos-i386.sxr") "SkyOS SXR runtime placeholder: skycrt`n"
    Write-TextFile (Join-Path $IsoRoot "pool\main\skyui_1.0.1.0.SKYCNQU_skyos-i386.sxr") "SkyOS SXR runtime placeholder: skyui`n"
    Write-TextFile (Join-Path $IsoRoot "pool\main\skygui-demo_1.0.1.0.SKYCNQU_skyos-i386.spk") "SkyOS SPK v2 GUI installer placeholder: skygui-demo`n"
    Write-TextFile (Join-Path $IsoRoot "pool\main\editor-vi_1.0.1.0.SKYCNQU_skyos-i386.spk") "SkyOS Sapp package placeholder: editor-vi`n"
    Write-TextFile (Join-Path $IsoRoot "pool\main\net-tools_1.0.1.0.SKYCNQU_skyos-i386.spk") "SkyOS Sapp package placeholder: net-tools`n"
    Write-TextFile (Join-Path $IsoRoot "pool\main\sapp-utils_1.0.1.0.SKYCNQU_skyos-i386.spk") "SkyOS Sapp package placeholder: sapp-utils`n"
    ...
    Write-TextFile (Join-Path $IsoRoot "drivers\DRIVERS.MANIFEST") "builtin: vga ps2 ide-pio sata-ahci-probe pci e1000 pc-speaker`nfuture: ahci-dma nvme rtl8139 virtio-net usb-hid framebuffer`n"
    ...
    Write-TextFile (Join-Path $IsoRoot "EFI\BOOT\README.txt") "UEFI directory reserved. BOOTIA32.EFI requires a future EFI loader build.`n"
}
