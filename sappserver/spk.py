...
def build_package(output, name, version=SKYOS_VERSION, internal_version=SKYOS_INTERNAL_VERSION,
                  ...
                  allow_write=None, language=None):
    ...
    if python_source and "python" not in languages:
        languages.append("python")
    if c_source and "c" not in languages:
        languages.append("c")
    if nasm_source and "nasm" not in languages:
        languages.append("nasm")
    if not runtime:
        if c_source or nasm_source:
            ...
            runtime = "soj"
    if runtime == "soj" and "soj" not in languages:
        languages.append("soj")
    if installer:
    ...
    meta = {
        ...
        "Standalone": bool(entry_point or commands),
        "CanInstallGUI": bool(gui or installer),
        "DependencyModel": "SXR",
        "TuringComplete": True,
        "Permissions": {
            "read": allow_read,
...
def build_index(repo, no_samples=False):
    ...
    sample_packages = [
        ...
        ("net-tools", "Network commands for SkyOS", "base-tools (>= 10015)"),
        ("editor-vi", "SkyOS vi editor package", "base-tools (>= 10015)"),
        ("sapp-utils", "Sapp package manager utilities", "base-tools (>= 10015), net-tools (>= 10015)"),
    ]
    if not no_samples:
    ...
    for file in sorted(list(pool.glob("*.spk")) + list(pool.glob("*.sxr"))):
        ...
        fmt = meta.get("Format") or meta.get("_format") or "spk-binary-v1"
        index_meta = dict(meta)
        entry = _meta_string(index_meta, "EntryPoint")
        commands = _meta_string(index_meta, "Commands")
        runtime = _meta_string(index_meta, "Runtime")
        package_type = _meta_string(index_meta, "PackageType")
        if file.suffix == ".sxr":
            index_meta.setdefault("Runtime", "sxr")
            index_meta.setdefault("PackageType", "sxr-library")
            index_meta.setdefault("RuntimeABI", "skyos-i386")
        elif fmt == "spk-binary-v2":
            if not runtime:
                runtime = "soj"
                index_meta["Runtime"] = runtime
            index_meta.setdefault("RuntimeABI", "skyos-i386")
            index_meta.setdefault("DependencyModel", "SXR")
            index_meta.setdefault("TuringComplete", True)
            if not index_meta.get("Languages"):
                if runtime == "python-bytecode":
                    index_meta["Languages"] = ["python"]
                elif runtime == "native-i386":
                    index_meta["Languages"] = ["c", "nasm"]
                else:
                    index_meta["Languages"] = ["soj"]
            executable = bool(entry or commands)
            index_meta.setdefault("Executable", executable)
            index_meta.setdefault("Standalone", executable)
            index_meta.setdefault("CanInstallGUI", package_type in ("installer", "gui-app") or bool(index_meta.get("GUI")) or bool(index_meta.get("Installer")))
        validate_depends(depends)
        validate_sxr_depends(sxr_depends)
        ...
        for key in ("Runtime", "RuntimeABI", "Languages", "EntryPoint", "Commands", "PackageType",
                    "Executable", "Standalone", "CanInstallGUI", "DependencyModel",
                    "Permissions", "TuringComplete"):
            value = _meta_string(index_meta, key)
            if value:
